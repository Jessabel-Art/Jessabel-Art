...
const NO_DRAG_SELECTOR =
  'button, a, input, select, textarea, [role="button"], [data-no-drag], .window-traffic, .window-menu-btn';
...
const PROJECTS = [
  {
    ...
    roleTitle:
      "Founder · Product Architect · Business Systems Designer · UI/UX & Implementation Lead",
    ...
    problem:
      "A service business can quickly accumulate disconnected pricing sheets, client records, invoices, service definitions, and administrative workflows. Alchemize was designed to establish a single operational foundation instead of allowing those systems to evolve independently.",
    architectureFlow: [
      {
        layer: "Public Experience",
        items: [
          "Services",
          "Service discovery",
      ...
      },
      {
        ...
        items: [
          ...
          "Client portal foundation (in progress)",
        ],
      },
      {
        layer: "Billing",
    ...
    architectureFuture: {
      layer: "Future System Extensions",
      items: [
        "Proposals / SOWs",
        "Expanded client portal views",
        "Deeper automation",
        "Additional recurring workflows",
      ],
    },
    canonicalService: {
      intro:
        "The Services module is designed as the canonical source of truth for the platform rather than duplicating service and pricing logic across individual screens.",
      hub: "Service Catalog",
    ...
    pricingModels: [
      ...
      "Future Expansion / Not Offered",
    ],
    pricingNote:
      "Different professional services require different commercial rules, so the platform’s data model was designed to accommodate those differences centrally rather than one-off, per-screen.",
    adminOperations: [
      ...
      "Administrative control of service availability",
    ],
    systemDesign:
      "Alchemize is structured so that core business objects relate to one another instead of existing as isolated UI features. Services and pricing can flow into client records and billing workflows, creating a foundation that can be extended without rebuilding the same business logic in each module.",
    techImplementation: [
      {
        label: "Front End",
        items: [
      ...
      },
      {
        label: "Data",
        items: [
          "Relational database, 40+ versioned SQL migrations",
          "Canonical service catalog schema",
          "Domain model spanning leads → clients → services → invoices → payments",
        ],
      },
      {
        label: "Application",
        items: [
          "Role-based admin accounts & invitations",
      ...
      },
      {
        label: "Deployment",
        items: [
          "Apache-hosted (.htaccess)",
          "GitHub Actions CI",
          "Automated a11y/security checks (Playwright, axe-core)",
          "Scripted deploy verification",
        ],
      },
    ],
    designDecisions: [
      {
        title: "One Service Source of Truth",
        body: "Pricing and service definitions should not be independently recreated inside Clients, Billing, Invoices, or future modules.",
      },
      {
        title: "Business Rules Belong in the System",
        body: "Different services require different pricing and authorization behaviors, so those rules are represented structurally rather than being buried only in interface copy.",
      },
      {
        title: "Designed for Extension",
        body: "The architecture is intended to allow proposals, recurring billing, client-facing service views, and other future modules to consume the same underlying service model.",
      },
    ],
    scopeSummary: {
    ...
    outcomeStatement:
      "One operational foundation connecting service definitions, pricing logic, client records, invoicing, payments, and future platform capabilities.",
  ...
  },
  {
    ...
    template: "cavalry",
    ownership:
      "Client branding and logo were supplied. My responsibility was translating that existing identity into the digital experience and designing and implementing the website and supporting business workflow.",
    overview: [
    ...
    problem:
      "A generic contact form cannot adequately capture what a property-service business needs in order to evaluate a job. Lawn care, cleanup, landscaping, hauling, and storm-cleanup requests all carry different details, so the system was designed to turn an unstructured inquiry into an organized, actionable quote request.",
    problemServices: [
      "Lawn Care",
      "Landscaping",
      "Property Cleanups",
    ...
    architectureNote:
      "The public quote form is connected to a server-side workflow. Submissions are validated, stored as structured quote-request records, associated job photos are processed separately, and the business receives a notification containing the information needed to follow up.",
    ...
    photoSafeguards: [
      ...
      "Extension assigned from validated MIME — not client input",
      "Safe, randomly generated filenames",
      "Directory-traversal protection",
      "Photos recorded separately from the quote request",
      "Quote + photo records written in a single database transaction",
    ],
    photoNote:
      "Customers can provide useful visual context while the server retains full control over what files are accepted and how they are stored.",
    notificationNote:
      "A successful submission generates a business notification containing the information needed to follow up quickly.",
    notificationFields: [
      "Request ID",
      "Customer Name",
      "Phone",
    ...
    techGroups: [
      {
        ...
        items: ["Responsive public website", "Static / Next.js export"],
      },
      { label: "Backend", items: ["PHP endpoint", "Server-side validation"] },
      {
        label: "Data",
        items: ["MariaDB", "quote_requests", "quote_request_photos"],
      },
      { label: "Notifications", items: ["PHPMailer", "SMTP"] },
      { label: "Infrastructure", items: ["Hostinger", "Production domain"] },
    ],
    outcomeStatement:
      "A production business website that functions as both a public presence and the first stage of Cavalry Green’s customer-acquisition workflow — turning an unstructured “contact us” interaction into organized, actionable quote requests.",
  ...
  },
  {
    ...
    template: "cleaning",
    ownership:
      "Built as a demo based on a real client engagement — not the client’s live production system.",
    overview: [
    ...
    roleTitle:
      "Product Architecture · UI/UX · Frontend Development · Booking & Business-Rule Logic",
    ...
    problem:
      "A service business needs more than a booking form. Availability, pricing, deposits, customer history, appointments, payments, invoices, and administrative controls all affect one another. The system was designed so those workflows operate as connected business processes rather than isolated screens.",
    compareBefore: ["Estimate", "Calendar", "Customer", "Payment", "Invoice"],
    compareAfter: ["Estimate", "Booking", "Customer", "Payment", "Operations"],
    journey: [
      {
        title: "Configure Service",
        body: "The customer provides property details and selects a service type.",
      },
      {
        title: "Estimate",
        body: "Pricing logic calculates an estimate from the configured business rules.",
      },
      {
        title: "Select Availability",
        body: "Available times are shown based on operating hours and day-closure rules.",
      },
      {
        title: "Book",
        body: "The appointment is created through the booking flow’s validated form logic.",
      },
      {
        title: "Deposit & Policy",
        body: "Deposit, hold-window, and cancellation policy are presented before confirmation.",
      },
      {
        title: "Service Record",
        body: "The booking becomes part of the operational and administrative workflow.",
      },
    ],
    pricingInputs: [
      {
        label: "Property",
        items: ["Bedrooms", "Bathrooms", "Square footage (office/commercial)"],
      },
      {
        label: "Service",
        items: [
          "Service type",
          "Deep-clean duration multiplier",
      ...
      },
      {
        label: "Condition",
        items: [
          "Property condition — light / standard / heavy",
          "Pets on site",
        ],
      },
      {
    ...
    pricingNote:
      "The estimate engine combines service configuration and business rules through one shared pricing module, rather than recalculating price logic in more than one place across the app.",
    bookingPipeline: [
      "Requested Date",
      "Operating Hours",
      "Day-Closure Rules",
      "Time-Slot Selection",
      "Form & Policy Validation",
      "Confirmed Booking",
    ],
    adminModules: [
      {
        label: "Dashboard",
        body: "Operational overview of bookings and activity.",
      },
      {
        label: "Bookings",
        body: "Appointment records and booking management.",
      },
      { label: "Calendar", body: "Operational scheduling visibility." },
      {
        label: "Clients",
        body: "Customer records with lifetime-value segmentation.",
      },
      {
        label: "Payments & Invoicing",
        body: "Invoice ledger, balances, and CSV export.",
      },
      {
        label: "Reports",
        body: "Revenue-by-service and booking-status reporting.",
      },
      { label: "Reviews", body: "Customer review and testimonial visibility." },
      {
        label: "Maintenance",
        body: "Local data and demo-environment health status.",
      },
    ],
    paymentFlow: [
      "Booking Submitted",
    ...
    paymentPolicies: [
      ...
      "Due within 24 hours or the slot may release",
      "One reschedule allowed with 48 hours notice",
      "Cancelling within 48 hours forfeits the deposit",
    ],
    canonicalIntro:
      "Pricing, booking, and invoice values were aligned around one shared data and rules layer so the customer-facing and administrative views never independently calculate or duplicate the same record.",
    canonicalFlow: ["Estimate", "Booking", "Invoice"],
    canonicalPrinciples: [
      "Canonical Pricing Rules",
      "Shared Booking & Invoice Data",
      "Admin / Client Parity",
      "Consistent Form Validation",
    ],
    accessNote:
      "Authentication in this public demo is a local, role-based session used to separate the customer and admin experiences. Production identity, data, and payment integrations were intentionally scoped out of the public repository and designed to sit behind a server-side boundary rather than ship real credentials or backend access inside a portfolio demo.",
    accessDetails: [
      { label: "Session Roles", value: "Client / Admin demo roles" },
      { label: "Route Separation", value: "/portal vs. /admin" },
      {
        label: "Repo Boundary",
        value: "No production secrets or backend config committed",
      },
      {
        label: "Production Path",
        value: "Real integrations designed for a server-side boundary",
      },
    ],
    architectureFlow: [
      "Customer UI — React + Vite",
      "Booking Form + Estimate Engine",
      "Local Demo Data Layer",
    ],
    architectureBranches: [
      "Admin Portal — Bookings · Calendar · Clients · Reports",
      "Client Portal — Appointments · Payment Center · Profile",
    ],
    techGroups: [
      {
        label: "Front End",
        items: ["React 18", "Vite", "React Router", "Tailwind CSS"],
      },
      {
        label: "UI System",
        items: [
      ...
      },
      {
        ...
        items: [
          "Canonical estimate / pricing module",
          "Booking + invoice generation",
          "Role-based demo sessions",
        ],
      },
      {
        label: "Admin Tooling",
        items: ["react-big-calendar", "Recharts reporting", "CSV export"],
      },
      {
        label: "Data & State",
        items: [
          "Local demo datasets",
          "sessionStorage pending bookings",
          "Shared records across customer & admin views",
        ],
      },
    ],
    designDecisions: [
      {
        title: "Shared Estimate Logic",
        body: "One pricing rules module is used everywhere an estimate is calculated, so customer and admin views can’t drift into inconsistent numbers.",
      },
      {
        title: "Designed Against Double-Booking",
        body: "Time-slot selection is scoped to operating-hours and closure rules the same way a production system would enforce availability server-side.",
      },
      {
        title: "Role-Separated Access",
        body: "Customer and admin experiences are gated by distinct session roles and routes rather than one view with hidden admin controls.",
      },
      {
        title: "Shared Data Across Surfaces",
        body: "Bookings, invoices, and reports all read from the same underlying records instead of independently recalculating customer-facing data.",
      },
    ],
    outcomeStatement:
      "A connected service-business platform where customer booking, pricing, scheduling, payment, and administrative operations operate as parts of the same workflow.",
    outcomeRelation: [
      "Customer Experience",
      "Booking Engine",
      "Business Logic",
  ...
  },
  {
    ...
    problem:
      "Personal financial information is often fragmented across bank accounts, budgeting tools, debt statements, investment accounts, savings goals, and long-term planning tools. FMBLifestyle explores a unified interface where those areas can be understood as parts of one financial picture.",
    ...
    coreExperiences: [
      {
        title: "Dashboard",
        body: "Consolidated overview of income, expenses, savings rate, balances, and recent transactions.",
      },
      {
        title: "Net Worth & Accounts",
        body: "Checking, savings, investment, vehicle, and real-estate balances alongside liabilities in one net-worth view.",
      },
      {
        title: "Budgeting",
        body: "Monthly category budgeting with spending visibility.",
      },
      {
        title: "Savings / Envelopes",
        body: "Goal-oriented savings buckets that separate financial objectives from general account balances.",
      },
      {
        title: "Debt",
        body: "Outstanding balances, APR, and payoff progress — including snowball/avalanche payoff planning — made visible alongside the rest of the financial picture.",
      },
      {
        title: "Mortgage Readiness",
        body: "A planning workspace for a future home purchase — readiness scoring, debt-to-income modeling, and loan-program comparisons. Not a lending qualification or approval tool.",
      },
    ],
    demoNote:
      "The portfolio version uses sample financial data and a local/offline-first data model. It demonstrates the product experience without connecting to real financial accounts or exposing personal financial information.",
    techGroups: [
      {
        label: "Front End",
      ...
      },
      {
        label: "Data",
        items: [
          "Local data service (localStorage)",
          "Offline-first, sample dataset",
      ...
      },
      {
        label: "Forward Path",
        items: [
          "Desktop-packaging groundwork (Tauri) present in the repository — not active in the web demo",
        ],
      },
    ],
    flowSteps: [
    ...
    outcomeStatement:
      "A unified financial-planning interface demonstrating how budgeting, net worth, debt, savings, and long-term financial goals can be presented as one connected financial picture.",
    outcomeRelation: [
      "Net Worth",
      "Accounts",
      "Budget",
...
const LAB_PROJECT = {
  ...
  description:
    "Custom ESP32 firmware driving a 128×64 SSD1306 OLED through a boot/dashboard/idle state machine — blending hand-converted bitmap scenes with live clock and weather data pulled over Wi-Fi.",
  techStack: [
    "ESP32",
    "C++",
    "Arduino",
  ...
  hardware:
    "ESP32 development board · SSD1306 128×64 OLED · I²C @ 0x3C (SDA → GPIO 21, SCL → GPIO 22, 3.3V/GND)",
  firmware: [
    { label: "Language", value: "C++, Arduino framework (Arduino IDE)" },
    {
      label: "Display",
      value: "Adafruit_SSD1306 + Adafruit_GFX over Wire (I²C)",
    },
    {
      label: "Connectivity",
      value:
        "WiFi + HTTPClient + ArduinoJson — NTP time sync and Open-Meteo weather",
    },
  ],
  graphicsPipeline:
    "13 custom 128×64 monochrome scenes (car, garage, glitch transitions, hello card) were converted to byte arrays and stored in PROGMEM flash, then blitted into the SSD1306 framebuffer with Adafruit GFX’s drawBitmap() — layered under stars, rain, snow, moonlight, and a vaporwave grid drawn pixel-by-pixel with GFX primitives.",
  displayLogic:
    "A six-state machine (boot, dashboard, cruise, night, garage, shutdown) runs on an ~80ms frame timer. Boot and shutdown play scripted, elapsed-time-gated sequences; the idle states loop weather-reactive effects around the bitmap scenes; a serial command interface (p / b / g / d / n / c / w) triggers any state directly for development.",
  installSteps: [
...
const ACHIEVEMENT_DEFINITIONS = {
  portfolioAccess: {
    ...
    body: "You opened the Projects app.",
  },
  hardwareDetected: {
    label: "HARDWARE DETECTED",
    body: "PinkLadyZ OLED signal found.",
  },
  ...
  fullSystemAccess: {
    ...
    body: "All primary apps discovered.",
  },
};
...
const PROJECT_ALIASES = {
  ...
  landscape: "landscape-website-demo",
  landscapewebsite: "landscape-website-demo",
  landscapewebsitedemo: "landscape-website-demo",
  "landscape-website-demo": "landscape-website-demo",
  pinkladyz: LAB_ALIAS,
  pinkladyzoled: LAB_ALIAS,
  "pinkladyz-oled": LAB_ALIAS,
  oled: LAB_ALIAS,
};
...
function applyPreferences() {
  document.body.classList.toggle(
    "pref-icons-off",
    !state.preferences.desktopIcons,
  );
  document.body.classList.toggle(
    "pref-discovery-off",
    !state.preferences.discoveryWidget,
  );
...
function updateDiscoveryIndicator() {
  ...
  if (discoveryChip) {
    ...
    `;
    discoveryChip.classList.toggle(
      "is-complete",
      count === PRIMARY_APP_IDS.length,
    );
  }
...
function registerAppOpen(appId) {
  ...
  if (
    getDiscoveryCount() === PRIMARY_APP_IDS.length &&
    !state.achievements.fullSystemAccess
  ) {
    unlockAchievement("fullSystemAccess");
  }
...
function renderCommandOutput() {
  ...
  commandOutput.innerHTML = state.commandLog
    .map(
      (entry) => `
...
function getDiscoverLines() {
  ...
  const rows = PRIMARY_APP_IDS.map((appId) => {
    const name =
      getAppDisplayName(appId) === LAB_PROJECT.name
        ? APP_CONFIG.lab.name
...
function getAchievementsLines() {
  ...
  return unlocked.map(
    (id) =>
      `${ACHIEVEMENT_DEFINITIONS[id].label} — ${ACHIEVEMENT_DEFINITIONS[id].body}`,
  );
}
...
function getWindowsLines() {
  ...
  const rows = ids.map(
    (id, index) =>
      `${String(index + 1).padStart(2, "0")}  ${state.windows.get(id).title}`,
  );
  return [
    "ACTIVE WINDOWS",
    "",
    ...rows,
    "",
    `${ids.length} window${ids.length === 1 ? "" : "s"} active.`,
  ];
}

function getRecentLines() {
  if (!state.recent.length) return ["No recent activity."];
  const rows = state.recent.map(
    (entry, index) => `${String(index + 1).padStart(2, "0")}  ${entry.name}`,
  );
  return ["RECENT", "", ...rows];
}
...
const COMMAND_DEFINITIONS = [
  ...
  {
    command: "whoami",
    description: "Show system profile",
    type: "info",
    run: () => getWhoamiLines(),
  },
  {
    ...
    type: "info",
    run: () => getDiscoverLines(),
  },
  {
    command: "achievements",
    description: "Show unlocked achievements",
    type: "info",
    run: () => getAchievementsLines(),
  },
  {
    command: "windows",
  ...
  {
    command: "recent",
    description: "Show recently opened items",
    type: "info",
    run: () => getRecentLines(),
  },
  {
  ...
  {
    command: "arrange",
    description: "Arrange open windows",
    type: "action",
    run: () =>
      arrangeWindows() ? "Windows arranged." : "No windows to arrange.",
  },
  {
    command: "minimize",
    ...
    type: "action",
    run: () =>
      minimizeActiveWindow() ? "Window minimized." : "No active window.",
  },
  {
    command: "close",
    description: "Close the active window",
    type: "action",
    run: () => (closeActiveWindow() ? "Window closed." : "No active window."),
  },
  {
    command: "desktop",
...
function updateCommandSuggestions(value = "") {
  ...
  const openHint =
    !query || "open".includes(query) || query.startsWith("open")
      ? [
          {
  ...
  commandSuggestions.innerHTML = combined
    .map(
      (entry) => `
  ...
  commandSuggestions
    .querySelectorAll(".command-suggestion")
    .forEach((button) => {
      button.addEventListener("click", () => {
...
function renderDock() {
  ...
  const openWindowIds = Array.from(state.windows.keys()).filter(
    (windowId) =>
      windowId !== "home" && APP_CONFIG[state.windows.get(windowId)?.appId],
  );
...
function updateDockState() {
  Array.from(dock.children).forEach((button) => {
    ...
    button.classList.toggle(
      "is-active",
      isFocused || (isOpenApp && !isMinimized),
    );
...
function getProjectPrimaryLabel(project) {
  ...
  if (project.id === "landscape-website-demo") return "Launch Demo";
  return "Launch Demo";
}
...
function renderProjectList(shell) {
  ...
  toolbar.querySelectorAll(".filter-tab").forEach((tab) => {
    tab.classList.toggle(
      "is-active",
      tab.dataset.filter === state.projectFilter,
    );
  });
  ...
  listContainer.innerHTML =
    ...
    `;
      ...
      .join("") ||
    '<p style="color:var(--text-muted); grid-column:1/-1;">No projects in this category yet.</p>';
...
function renderStandardProjectDetail(
  project,
  displayBadge,
  primaryButtonLabel,
) {
  ...
  const capabilityGroupsMarkup = project.capabilityGroups
    .map(
      (group) => `
  ...
  const techChipsMarkup = project.tech
...
const ARCH_FORK_SVG =
  '<svg viewBox="0 0 200 40" preserveAspectRatio="none" aria-hidden="true"><path d="M100 0 L26 40 M100 0 L174 40" fill="none" stroke="currentColor" stroke-width="2"/></svg>';

function renderExtendedProjectDetail(
  project,
  displayBadge,
  primaryButtonLabel,
) {
  ...
  const [publicExperience, serviceCatalog, clientOperations, billing] =
    project.architectureFlow;
  const archDiagramLabel = [
    publicExperience,
    serviceCatalog,
    clientOperations,
    billing,
    ...
    <div class="canonical-arch-grid">
      ...
      <div class="canonical-arch-right">
        <div class="service-tree">
          ...
          <div class="service-tree-trunk" aria-hidden="true"></div>
          <div class="service-tree-node service-tree-future">${cs.future
            .split("\n")
  ...
  const techImplMarkup = project.techImplementation
  ...
  const outcomeSequence = project.architectureFuture
    ? [
        serviceCatalog.layer,
        clientOperations.layer,
        billing.layer,
        "Extensible Platform",
      ]
    : null;
  const outcomeFlowMarkup = outcomeSequence
    ? outcomeSequence
        .map(
          (node, index, arr) => `
  ...
  `,
        ...
        .join("")
    : "";
  const outcomeLaunchLink = project.url
...
function renderCleaningServiceDetail(
  project,
  displayBadge,
  primaryButtonLabel,
) {
  ...
  const pricingInputsMarkup = project.pricingInputs
    .map(
      (group) => `
    <div class="detail-capability-group">
      <span class="detail-capability-label">${group.label}</span>
  ...
  const pricingFlowInputsMarkup = project.pricingFlow
    .map(
      (item, index, arr) => `
    <span class="chip">${item}</span>
  ...
  const bookingPipelineMarkup = project.bookingPipeline
    .map(
      (step, index, arr) => `
  ...
  const adminModulesMarkup = project.adminModules
    .map(
      (module) => `
  ...
  const paymentFlowMarkup = project.paymentFlow
    .map(
      (step, index, arr) => `
    <div class="pipeline-node ${index === arr.length - 1 ? "is-final" : ""}">${step}</div>
    ${index < arr.length - 1 ? '<div class="arch-arrow" aria-hidden="true">&darr;</div>' : ""}
  ...
  const paymentPoliciesMarkup = project.paymentPolicies
    .map((item) => `<span class="chip">${item}</span>`)
    .join("");

  const canonicalChainMarkup = project.canonicalFlow
    .map(
      (node, index, arr) => `
    <span class="flow-chain-node">${node}</span>
    ${index < arr.length - 1 ? '<span class="flow-chain-arrow" aria-hidden="true">&rarr;</span>' : ""}
  ...
  const canonicalPrinciplesMarkup = project.canonicalPrinciples
    .map((item) => `<span class="chip">${item}</span>`)
    .join("");

  const accessDetailsMarkup = project.accessDetails
    .map(
      (item) => `
  ...
  const architectureFlowMarkup = project.architectureFlow
    .map(
      (step, index, arr) => `
  ...
  const architectureBranchesMarkup = project.architectureBranches
    .map((branch) => `<div class="pipeline-node">${branch}</div>`)
    .join("");

  const techGroupsMarkup = project.techGroups
    .map(
      (group) => `
    <div class="detail-capability-group">
      <span class="detail-capability-label">${group.label}</span>
...
function renderCavalryDetail(project) {
  const overviewMarkup = project.overview
    .map((paragraph) => `<p>${paragraph}</p>`)
    .join("");
  const roleItemsMarkup = project.roleItems
    .map((item) => `<span class="chip">${item}</span>`)
    .join("");
  const problemServicesMarkup = project.problemServices
    .map((item) => `<span class="chip">${item}</span>`)
    .join("");
  ...
  const photoFormatsMarkup = project.photoFormats
    .map((item) => `<span class="chip">${item}</span>`)
    .join("");
  const photoLimitsMarkup = project.photoLimits
    .map(
      (item) => `
    <div class="meta-strip-item"><span>${item.label}</span><strong>${item.value}</strong></div>
  `,
    ...
    .join("");
  const photoSafeguardsMarkup = project.photoSafeguards
    .map((item) => `<span class="chip">${item}</span>`)
    .join("");

  const notificationFieldsMarkup = project.notificationFields
    .map((item) => `<span class="chip">${item}</span>`)
    .join("");
  const notificationTechMarkup = project.notificationTech
    .map((item) => `<span class="chip tech-chip">${item}</span>`)
    .join("");

  const seoGroupsMarkup = project.seoGroups
    .map(
      (group) => `
    <div class="detail-capability-group">
      <span class="detail-capability-label">${group.label}</span>
  ...
  const plateClass = project.logoDark
    ? "project-plate is-dark"
    : "project-plate";
...
function renderFMBDetail(project, displayBadge) {
  ...
  const architectureInputsMarkup = project.architectureInputs
    .map(
      (item, index, arr) => `
    <span class="chip">${item}</span>
    ${index < arr.length - 1 ? '<span class="mini-flow-plus" aria-hidden="true">+</span>' : ""}
  ...
  const architectureModulesMarkup = project.architectureModules
    .map((item) => `<div class="pipeline-node">${item}</div>`)
    .join("");

  const coreExperiencesMarkup = project.coreExperiences
    .map(
      (card) => `
    <div class="admin-module-card">
  ...
  const flowStepsMarkup = project.flowSteps
    .map(
      (step, index, arr) => `
    <div class="pipeline-node ${index === arr.length - 1 ? "is-final" : ""}">${step}</div>
    ${index < arr.length - 1 ? '<div class="arch-arrow" aria-hidden="true">&darr;</div>' : ""}
  ...
  const flowModulesMarkup = project.flowModules
    .map((item) => `<span class="chip">${item}</span>`)
    .join("");
...
function renderSantosDetail(project, displayBadge) {
  ...
  const currentBuildMarkup = project.currentBuild
    .map(
      (item) => `
    <div class="admin-module-card">
  ...
  const structureMarkup = project.structureNodes
    .map(
      (node, index, arr) => `
    <span class="flow-chain-node">${node}</span>
    ${index < arr.length - 1 ? '<span class="flow-chain-arrow" aria-hidden="true">&rarr;</span>' : ""}
  ...
  const activeFocusMarkup = project.activeFocus
    .map((item) => `<span class="chip">${item}</span>`)
    .join("");
  const nextStageMarkup = project.nextStage
...
function createProjectDetailWindow(projectId) {
  ...
  const plateClass = [
    "project-plate",
    project.logoDark && "is-dark",
    project.logoWide && "is-wide",
  ]
    .filter(Boolean)
    .join(" ");
...
function renderLabWindow() {
  ...
  const flowMarkup = LAB_PROJECT.flowNodes
    .map((node, index) => {
      const arrow =
        index < LAB_PROJECT.flowNodes.length - 1
          ? '<span class="lab-flow-arrow" aria-hidden="true">&darr;</span>'
          : "";
...
const ABOUT_PROFILE = {
  ...
  statementPrimary:
    "I build technology around how businesses actually operate.",
  statementBody: [
  ...
  foundationSynthesis:
    "Business understanding + design training + technical implementation.",
  techGroups: [
    {
  ...
  whyLede:
    "My advantage isn’t technology in isolation. It’s understanding the business on the other side of it.",
  whyBody: [
  ...
  founderCopy:
    "I apply this approach through Alchemize Business Services, where business strategy, service architecture, digital experiences, administrative operations, pricing, client management, billing, and automation are developed as connected parts of a broader business platform.",
  systemWork: [
    {
      kind: "project",
      id: "alchemize",
    ...
    },
    {
      ...
      name: "Cleaning Service Demo",
      descriptor:
        "Service operations, CRM, booking, payments, invoicing & administration",
    },
    {
      kind: "project",
      id: "cavalry-green",
      name: "Cavalry Green LLC",
      descriptor: "Production client website & quote-request workflow",
    },
    {
      kind: "lab",
      id: null,
      name: "PinkLadyZ OLED",
      descriptor: "ESP32 embedded display system",
    },
    {
      kind: "system",
      id: null,
      name: "Jessabel OS",
      descriptor: "Interactive portfolio system",
    },
  ],
};
...
function renderAboutWindow() {
  ...
  const foundationMarkup = ABOUT_PROFILE.foundation
    .map(
      (card) => `
  ...
  const techGroupsMarkup = ABOUT_PROFILE.techGroups
    .map(
      (group) => `
    <div class="detail-capability-group">
      <span class="detail-capability-label">${group.label}</span>
  ...
  const flowChainMarkup = ABOUT_PROFILE.flowChain
    .map(
      (node, index, arr) => `
    <span class="flow-chain-node">${node}</span>
    ${index < arr.length - 1 ? '<span class="flow-chain-arrow" aria-hidden="true">&rarr;</span>' : ""}
  ...
  const systemWorkMarkup = ABOUT_PROFILE.systemWork
    .map((entry) => {
      const attr =
        entry.kind === "project"
          ? `data-project-id="${entry.id}"`
          : `data-open-app="${entry.kind}"`;
      return `
...
function calculateWindowPosition(windowData) {
  ...
  const offsetIndex = Array.from(state.windows.values()).filter(
    (entry) => entry.appId !== "home",
  ).length;
  const baseX = Math.min(180 + (offsetIndex % 3) * 16, window.innerWidth - 260);
  const baseY = Math.min(
    100 + (offsetIndex % 3) * 14,
...
function openWindow(windowData) {
  ...
  const position = {
    x: Math.min(
      Math.max(rawPosition.x, 16),
      Math.max(16, window.innerWidth - safeWidth - 16),
    ),
    y: Math.min(
      Math.max(rawPosition.y, 46),
      Math.max(46, window.innerHeight - safeHeight - 16),
    ),
  };
  ...
  state.windows.set(windowData.id, {
    ...windowData,
...
function setMaximizeControlLabel(element, isMaximized) {
  ...
  menuButton?.setAttribute(
    "aria-label",
    isMaximized ? "Restore window size" : "Toggle window size",
  );
}
...
function getUsableDesktopRect() {
  ...
  const dockReserve =
    state.preferences.dock && dockEl && !dockEl.classList.contains("hidden")
      ? 84
      : 20;
  return {
    top: 48,
...
function centerWindow(windowId) {
  ...
  const left =
    usable.left + Math.max(0, (usable.right - usable.left - width) / 2);
  const top =
    usable.top + Math.max(0, (usable.bottom - usable.top - height) / 2);
...
function arrangeWindows() {
  const usable = getUsableDesktopRect();
  const ids = Array.from(state.windows.keys()).filter(
    (id) => !state.minimized.has(id),
  );
...
function getSplitViewCandidates() {
  ...
  return SPLIT_CANDIDATE_APP_IDS.filter((appId) => appId !== activeAppId).map(
    (appId) => ({
      appId,
...
const PREFERENCE_ROWS = [
  {
    key: "desktopIcons",
    label: "Desktop Icons",
    hint: "Show the app launcher icons on the desktop.",
  },
  {
    key: "discoveryWidget",
    label: "Discovery Widget",
    hint: "Show the growing-plant discovery indicator.",
  },
  { key: "dock", label: "Dock", hint: "Show the application dock." },
  {
    key: "reduceMotion",
    label: "Reduce Motion",
    hint: "Minimize transitions and animation.",
  },
];
...
function setupWindowDrag(windowEl, windowId) {
  ...
  header.addEventListener("pointermove", (event) => {
    ...
    const nextLeft = Math.min(
      Math.max(dragInfo.startLeft + deltaX, 16),
      window.innerWidth - 180,
    );
    const nextTop = Math.min(
      Math.max(dragInfo.startTop + deltaY, 46),
...
homeMenu?.querySelectorAll("[data-home-action]").forEach((button) => {
  button.addEventListener("click", () => {
    ...
    if (action === "discovery") {
      showToast(
        "SYSTEM DISCOVERY",
        `${getDiscoveryCount()} / ${PRIMARY_APP_IDS.length} discovered`,
      );
    }
    if (action === "achievements") {
      const list = Object.keys(state.achievements).length
...
function downloadResume() {
  showToast(
    "RESUME",
    "No resume file on record yet — opening an email request instead.",
  );
...
function buildFileMenuItems() {
  ...
  return [
    ...
    {
      label: "Close Window",
      shortcut: `${getPrimaryModifierLabel()} W`,
      disabled: !hasActive,
      onClick: () => closeActiveWindow(),
    },
    {
      label: "Close All Windows",
      disabled: !hasAnyWindow,
...
function buildViewMenuItems() {
  return [
    ...
    { separator: true },
    {
      ...
      checked: state.preferences.desktopIcons,
      stayOpen: true,
      onClick: () =>
        setPreference("desktopIcons", !state.preferences.desktopIcons),
    },
    {
      label: "Show Discovery",
      checked: state.preferences.discoveryWidget,
      stayOpen: true,
      onClick: () =>
        setPreference("discoveryWidget", !state.preferences.discoveryWidget),
    },
    {
      label: "Show Dock",
      checked: state.preferences.dock,
      stayOpen: true,
      onClick: () => setPreference("dock", !state.preferences.dock),
    },
    { separator: true },
    {
      label: isFullScreenActive() ? "Exit Full Screen" : "Enter Full Screen",
...
function buildWindowMenuItems() {
  const hasActive = Boolean(state.activeWindowId);
  const activeRecord = hasActive
    ? state.windows.get(state.activeWindowId)
    : null;
  const isMaximized = Boolean(
    activeRecord?.windowElement.classList.contains("is-maximized"),
  );
  ...
  const windowList = Array.from(state.windows.keys()).map((id) => ({
    ...
    onClick: () =>
      state.minimized.has(id) ? restoreWindow(id) : focusWindow(id),
  }));
  ...
  const splitItems = !splitSupported
    ? [
        {
          label:
            window.innerWidth < SPLIT_VIEW_MIN_WIDTH
              ? "Unavailable at this window size"
              : "Open a window first",
          disabled: true,
        },
      ]
    : candidates.length
      ? candidates.map((candidate) => ({
          label: `Split with ${candidate.name}`,
          onClick: () => splitView(candidate.appId),
        }))
  ...
  return [
    {
      label: "Minimize",
      disabled: !hasActive,
      shortcut: `${getPrimaryModifierLabel()} M`,
      onClick: () => minimizeActiveWindow(),
    },
    {
      label: isMaximized ? "Restore" : "Maximize",
      disabled: !hasActive,
      onClick: () => toggleMaximizeActiveWindow(),
    },
    {
      label: "Center",
      disabled: !hasActive,
      onClick: () => centerWindow(state.activeWindowId),
    },
    { separator: true },
    {
      label: "Tile Left",
      disabled: !hasActive,
      onClick: () => tileWindow(state.activeWindowId, "left"),
    },
    {
      label: "Tile Right",
      disabled: !hasActive,
      onClick: () => tileWindow(state.activeWindowId, "right"),
    },
    { separator: true },
    {
      submenu: true,
...
function openSystemDropdown(menuKey, buttonEl) {
  ...
  const firstItem = systemDropdown.querySelector(
...
function closeSystemDropdown() {
  ...
  document
    .querySelectorAll(".system-menu-item[aria-expanded]")
...
systemDropdown?.addEventListener("keydown", (event) => {
  const items = Array.from(
    systemDropdown.querySelectorAll(".dropdown-item:not([disabled])"),
  );
  ...
  if (event.key === "ArrowDown") {
    ...
    closeSystemDropdown();
    document
      .querySelector(`.system-menu-item[data-menu="${menuKey}"]`)
      ?.focus();
  }
});
...
document.addEventListener("click", (event) => {
  if (!state.activeDropdown) return;
  if (
    event.target.closest("#systemDropdown") ||
    event.target.closest(".system-menu-item")
  )
    return;
  closeSystemDropdown();
});
...
commandInput?.addEventListener("input", (event) =>
  updateCommandSuggestions(event.target.value),
);
commandInput?.addEventListener("keydown", (event) => {
  if (event.key === "Enter") {
    ...
    if (!state.commandHistory.length) return;
    event.preventDefault();
    state.commandHistoryIndex = Math.min(
...
document.addEventListener("click", (event) => {
  ...
  if (
    event.target.closest("#homeMenu") ||
    event.target.closest("#desktopHomeButton")
  )
    return;
  homeMenu.classList.add("hidden");
  state.homeMenuOpen = false;
...
if (searchTrigger) {
  searchTrigger.setAttribute(
    "aria-label",
    `Open command palette (${getPrimaryModifierLabel()} + K)`,
  );
}
