...
// Compact the header once the page scrolls past the utility bar, and mark
// the hero-adjacent header as "over dark art" so its nav stays legible when
// it starts out on top of a photograph instead of the cream background.
...
document.querySelectorAll('[data-portfolio]').forEach(el=>el.onclick=()=>{try{const url=new URL(business.portfolioUrl);if(url.protocol==='https:'||url.protocol==='http:'){window.open(url.href,'_blank','noopener,noreferrer');return;}}catch{}openDialog('<div class="dialog-copy"><p class="label">Portfolio demonstration</p><h2 id="dialog-title">You’re exploring a work sample.</h2><p>The portfolio return destination has not been connected yet. This standalone demo showcases the Alder & Field website experience.</p><button class="button" onclick="this.closest(\'dialog\').close()">Keep exploring<span class="button-arrow" aria-hidden="true"></span></button></div>');});
document.querySelector('[data-demo-info]').onclick=()=>openDialog('<div class="dialog-copy"><p class="label">About this website</p><h2 id="dialog-title">A fictional brand.<br>A complete experience.</h2><p>Alder & Field is a portfolio demonstration, not an operating business. Landscapes are illustrative concepts, and contact details and business hours are examples. No customer testimonials or business credentials are claimed.</p><h3>Your privacy</h3><p>Quote submissions are simulated in your browser. Personal information and photos are not transmitted or saved. Material selections are stored on this device so you can browse between pages. No analytics or marketing cookies are used.</p><button class="button" id="clear-preferences">Clear saved materials<span class="button-arrow" aria-hidden="true"></span></button></div>');
